...
public class RestaurantDetailActivity extends AppCompatActivity
    ...
    private Task<Void> addRating(final DocumentReference restaurantRef, final Rating rating) {
        ...
        return mFirestore.runTransaction(new Transaction.Function<Void>() {
            //add the new rating and update the aggregate rating
            @Nullable
            @Override
            public Void apply(@NonNull Transaction transaction) throws FirebaseFirestoreException {

                Restaurant restaurant= transaction.get(restaurantRef).toObject(Restaurant.class);
                double oldTotalRating= restaurant.getAvgRating() * restaurant.getNumRatings();
                int newNumberOfRatings= restaurant.getNumRatings() + 1;

                double newAggregateRating= (oldTotalRating + rating.getRating())/ newNumberOfRatings;

                restaurant.setNumRatings(newNumberOfRatings);
                restaurant.setAvgRating(newAggregateRating);
